...
- 사소하지 않은 작업은 완료 전에 서브 에이전트 코드 리뷰를 받습니다.
- 사소하지 않은 작업에는 기능 추가, 버그 수정, 리팩터링, 빌드/배포 설정 변경, 데이터 흐름 변경, 보안/성능에 영향을 줄 수 있는 변경이 포함됩니다.
- 리뷰용 서브 에이전트에는 변경 diff와 검증 결과를 제공하고, 정확성, 회귀 위험, 누락된 테스트, 보안/개인정보 이슈를 우선 확인하게 합니다.
- 리뷰 지적이 타당하면 수정한 뒤 필요한 검증을 다시 실행합니다.
- 서브 에이전트 리뷰를 실행할 수 없는 환경이면 최종 응답에 그 사실을 밝히고, 리뷰를 완료한 것처럼 말하지 않습니다.
- 문서 오탈자 수정, 주석 보강, 포맷만 바꾸는 작은 변경은 서브 에이전트 리뷰를 생략할 수 있습니다.
