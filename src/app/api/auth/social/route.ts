...
export async function POST(request: Request) {
    try {
        ...
        const parsed = z.object({ refreshToken: z.string().min(1).max(3000) }).safeParse(await request.json().catch(() => null));
        if (!parsed.success) return NextResponse.json({ message: "로그인 정보를 확인해 주세요." }, { status: 400, headers: PRIVATE_HEADERS });
        const { session, isNewUser } = await createSession(parsed.data.refreshToken);
