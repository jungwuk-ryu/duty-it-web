...
import styles from "./smoky-button.module.css";
...
const toSettings = (colors: SmokeColors, speed: number): SmokeSettings => ({
...
export const SmokyButton = forwardRef<HTMLButtonElement, SmokyButtonProps>(
  (
    ...
    useEffect(() => {
      ...
      try {
        rendererRef.current = createSmokeRenderer(canvas, toSettings(DEFAULT_SMOKE_COLORS, 0.72));
      } catch {
    ...
    useEffect(() => {
      rendererRef.current?.update({
        colors: [hexToRgb(primary), hexToRgb(secondary), hexToRgb(shadow)],
        speed,
      });
    }, [primary, secondary, shadow, speed]);
    ...
    return (
      <button
        ...
        ref={ref}
        type={type}
        className={cn(styles.smokyButton, className)}
      >
