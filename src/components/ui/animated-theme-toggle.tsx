...
export function AnimatedThemeToggle({ className }: { className?: string }) {
    ...
    const label = mounted ? (isDark ? "라이트 모드로 전환" : "다크 모드로 전환") : "화면 테마 전환";
    ...
    return (
        ...
        >
            {mounted ? <SolarSwitch isDark={isDark} /> : <span className="size-5" aria-hidden="true" />}
        </Button>
    );
}
...
function SolarSwitch({ isDark }: { isDark: boolean }) {
    ...
    return (
        <svg width="20" height="20" viewBox="0 0 25 25" fill="none" aria-hidden="true" focusable="false">
            {SUN_PATHS.map((path) => (
                <motion.path
                    key={path}
                    d={path}
