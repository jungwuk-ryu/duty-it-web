...
export default function HomeEventMarquee({ events }: Props) {
    ...
    return (
        <div className="relative h-[25rem] overflow-hidden rounded-3xl border border-slate-200/80 bg-white [perspective:720px] sm:h-[27rem]">
            <div
    ...
    return (
        ...
        >
            ...
            <div className="flex min-h-[6.5rem] flex-col gap-2 p-3">
                <p className="line-clamp-2 text-sm font-bold leading-5 text-slate-900">{event.title}</p>
                <div className="mt-auto flex items-center justify-between gap-2 text-xs">
                    <span className="truncate font-semibold text-brand">{EventTypeLabel[event.eventType]}</span>
                    <time className="shrink-0 text-slate-500" dateTime={event.startAt.toISOString()}>
                        {KST_DATE_FORMATTER.format(event.startAt)}
                    </time>
                </div>
                <p className="truncate text-xs text-slate-500">{event.host.name}</p>
            </div>
        </Link>
    );
}
