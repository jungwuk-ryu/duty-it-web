...
export function Header() {
    ...
    return (
        <header
            className={cn(
                "sticky top-0 z-50 mx-auto w-full border border-transparent bg-background/90 backdrop-blur-md transition-[width,max-width,top,border-radius,background-color,box-shadow,border-color] duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] md:max-w-[calc(100%-0px)] md:rounded-none",
                scrolled && !open && "top-3 w-[calc(100%-1.5rem)] rounded-2xl border border-border/80 bg-background/80 shadow-xl shadow-slate-950/[0.08] backdrop-blur-xl md:top-4 md:w-[calc(100%-2rem)] md:max-w-5xl md:rounded-3xl",
        ...
        >
            <nav className="relative z-10 mx-auto flex h-16 w-full max-w-[1280px] items-center justify-between px-5 transition-[padding] duration-500 md:h-20 md:px-8 lg:px-10" aria-label="주요 메뉴">
                <Link href="/" className="flex items-center gap-1.5 text-xl font-bold tracking-tight text-foreground" onClick={closeMenu}>
