        ...
        <div className="flex flex-col gap-12 pb-8 sm:gap-16" data-events-view="explore">
            <section className="flex flex-col items-center gap-7 py-6 text-center sm:gap-9 sm:py-10" aria-labelledby="events-heading">
                <header className="flex flex-col gap-3">
                    <h1 id="events-heading" tabIndex={-1} className="text-balance text-3xl font-bold tracking-tight text-slate-950 outline-none sm:text-5xl sm:leading-tight">
                        나의 다음 <span className="text-brand">간호 대외활동</span>을 찾아보세요
                    </h1>
                    <p className="text-sm leading-7 text-slate-600 sm:text-base">배움부터 새로운 도전까지, 나에게 맞는 행사를 한곳에서 찾아봐요.</p>
                </header>
                <EventSearch
                    hero
                    value={searchKeyword}
            ...
            <section aria-labelledby="events-preview-title">
                <header className="mb-6 flex items-center justify-between gap-4">
                    <div className="flex flex-col gap-2">
                        <h2 id="events-preview-title" className="text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl">새로 올라온 행사</h2>
                        <p className="text-sm text-slate-600">지금 참여할 수 있는 기회를 만나보세요.</p>
                    </div>
                    <Link
